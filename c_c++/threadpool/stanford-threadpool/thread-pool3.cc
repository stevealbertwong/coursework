...
num of threads: => reizing, thrashing, cache, slow down when more threads
multi-process server, why partition server into processes not threads?
	=> isolation, 1 process crash wont affect server
	=> not much commuincation between independent request, no need for sychronization
	=> security: different address space if exploit 
	=> protection/isolation: other crap non-threadsafe code/library from internet crash

dispatcher 
	=> like load balancer, maintain data structure of available servers and estimated load 
	=> balance each server/process/thread's work load
	=> decide who gets the work, distribute requests to a pool of web servers
	=> interrequest parallelism: depends on the function, several threads work tgt

worker should be stateless
	=> scale up and down workers, not over-provisioning

put front end cache in between load balancer n web server
...
void ThreadPool::worker(size_t id) {
	while (true) {
		...
		workers[id].thunk(); // function<void(void)>
		cout << id << endl;
