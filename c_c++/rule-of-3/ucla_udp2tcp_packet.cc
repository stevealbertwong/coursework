...
TCP_Packet::TCP_Packet(const TCP_Packet& other)
    : m_data(other.m_data), m_enc_count(other.m_enc_count), 
    m_sent(other.m_sent), m_acked(other.m_acked)
    
{
    ...
    // memcpy(this->m_encoded_packet, str.m_encoded_packet, this->m_enc_count);
}
